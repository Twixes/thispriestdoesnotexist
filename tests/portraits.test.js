...
test('reloads choose existing portraits without immediate repeats', async () => {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    assert.match(image.src, /^\/portraits\/(0[1-9]|[1-7][0-9]|80)\.webp$/);
...
test('all eighty numbered portraits are included in the deployment', () => {
  const files = readdirSync(new URL('../public/portraits/', import.meta.url)).filter(name => name.endsWith('.webp')).sort();
  assert.deepEqual(files, Array.from({ length: 80 }, (_, i) => `${String(i + 1).padStart(2, '0')}.webp`));
});
