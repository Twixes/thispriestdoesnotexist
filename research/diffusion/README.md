...
**The owner rejected the baseline's photographic realism.** The adapter mainly changes framing/collars, without a material realism or identity-variety improvement. Independent engineering pass labels in the detailed reviews do not override the owner's assessment. The [saved decision](../reviews/juggernaut-development/decision-after20.json) stops this pilot at20; no100 run, sealed test or promotion. Full non-accelerated Juggernaut X v10 is the next quality reference, with its own verified weights and author-recommended longer sampling.
...
Retain SDXL-Turbo512 as the speed reference, Juggernaut Hyper native1024 as the photographic SDXL candidate, and FLUX.2-klein4B as the documented base-to-distilled adapter route. Turbo's completed ordinary LoRA pilot did not improve quality. Juggernaut is easier to try locally than FLUX because the SDXL training path has already completed actual optimizer updates within this machine's resource bounds, but training at native1024 and preserving Hyper distillation remain unproven. Do not transfer the failed Turbo adapter or reuse Turbo VAE/text caches. The decoder probe found no material backend difference; test broader identity variety and a fresh small adapter against this exact base, preserving a base-only control. For SDXL, select a post-training method that preserves one/few-step sampling. Ordinary all-timestep LoRA fine-tuning is not proven to retain Turbo's distilled behavior. An alternative is a domain LoRA on ordinary SDXL with a released acceleration adapter; that composition still needs actual quality/latency evaluation. Do not claim post-training has happened just because the base already generates priests.
