...
`evaluate_sdxl_pilot.py --cached-prompts` is a prepared alternative for the four-step fallback. It saves the same FP16 text conditioning before releasing both encoders, then loads the image model. The default evaluation mode stays unchanged. Cached-mode startup and timing exclusions are explicit, with per-prompt tensor hashes for matching base and adapter runs; this new path has source checks only and has not yet run. The35% start/20% runtime memory reserves remain unchanged. The [fallback preview](../reviews/sdxl-pilot-4step-preview/index.html) currently displays an incomplete run rather than invented results.
