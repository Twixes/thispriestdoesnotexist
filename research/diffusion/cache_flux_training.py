...
def worker(output):
    ...
    def guard(phase):
        ...
        if record["mps_driver_bytes"] > 22*1024**3 or record["rss_bytes"] > 24*1024**3 or record["available_fraction"] < .20 or record["swap_growth_bytes"] > 512*1024**2:
    ...
    encoder = Flux2KleinPipeline.from_pretrained(model, transformer=None, vae=None,
        torch_dtype=torch.bfloat16, local_files_only=True).to("mps")
    ...
    with torch.inference_mode():
        for index in range(len(dataset)):
            ...
            stored_embeds = embeds.to(device="cpu", dtype=torch.float16).contiguous()
            if not torch.isfinite(stored_embeds).all().item():
                raise FloatingPointError(f"Embedding FP16 conversion overflow at{index}")
            encoded.append({"prompt_embeds": stored_embeds, "text_ids": ids.cpu().contiguous()})
    ...
    write(output / "manifest.json", {"complete": True, "entries": entries,
        ...
        "text_encoder_compute_dtype":"bfloat16", "prompt_embedding_storage_dtype":"float16",
        "numeric_recipe_note":"Frozen Qwen uses native BF16 weights to avoid a second converted weight allocation; finite outputs cast toFP16 for the upstream FP16 training transformer. This differs numerically from upstream all-FP16 caption encoding.",
        "resource_policy_note":"20%available floor after prior BF16 probe measured8.74GiBdriver,0.49GiBRSS and10.87GiBremaining; retains22GiBdriver/24GiBRSS caps and512MiBswap-growth stop. Historical25%guard failures retained.",
        ...
        try:
            while process.poll() is None:
                ...
                if peak > 24*1024**3:
                    ...
                    failure = "Available memory below20%"
