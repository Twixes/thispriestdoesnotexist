...
def worker(args):
    ...
    prompts = [args.prompt_prefix + prompt for prompt in PROMPTS]
    ...
    write(args.output / "result.json", {"complete": True, "model": "flux2-klein-4b",
        ...
        "server_latency_proven": False, "post_trained": bool(args.lora), "all_outputs_included": True,
        "adapter_sha256": sha(args.lora) if args.lora else None,
...
def main():
    ...
    write(args.output / "protocol.json", {"model": "flux2-klein-4b", "steps": 4, "count": 8,
        "prompts": [args.prompt_prefix+p for p in PROMPTS], "seed_base": 202609214000, "warmup_seed": 202609213999,
        "adapter_sha256": sha(args.lora) if args.lora else None,
    ...
    with (args.output / "worker.log").open("w") as log:
        command = [sys.executable, str(Path(__file__)), "--resolution", str(args.resolution),
            "--output", str(args.output), "--prompt-prefix", args.prompt_prefix, "--worker"]
        if args.lora:
            command.extend(["--lora", str(args.lora)])
