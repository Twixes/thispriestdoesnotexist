"""Full non-Hyper Juggernaut X v10 quality reference, DPM++2M Karras35/CFG3.

All original eight prompts/seeds and warmup at native square1024. Cached CLIPs
are released before direct-MPS UNet/VAE loading. No LoRA, repairs or selection.
"""
import argparse
import gc
import hashlib
...
MODEL = HERE / "models/juggernaut-x-v10"
REVISION = "35e4f3ccbe745c3e1221715d04a452aaa1ee5761"
PROVENANCE_SHA256 = "154d5e4735df58e6af795657e19a436b540dafdd1d6cb34374ab03befce64e70"
CHECKPOINT_SHA256 = "d91d35736d8f2be038f760a9b0009a771ecf0a417e9b38c244a84ea4cb9c0c45"
COMPONENTS = HERE / "models/juggernaut-x-v10-components"
COMPONENT_PROVENANCE_SHA256 = "4e18b462e1e07d35fb753ecb89dadabfd2d66213646f30467ee351f2bb36b40a"
...
LIMIT_SECONDS = 1200
GIB = 1024 ** 3
QUALITY_SCHEDULER = {"algorithm_type": "dpmsolver++", "solver_order": 2, "solver_type": "midpoint",
                     "use_karras_sigmas": True, "lower_order_final": True, "final_sigmas_type": "zero",
...
def write(path, value):
    ...
    temporary.write_text(json.dumps(json_finite(value), indent=2, allow_nan=False) + "\n")
...
def preflight(args):
    require(args.cached_text and args.case_manifest is None and args.lora is None and not args.prompt_prefix and args.native_review is None, "Full-base quality reference is exactly original8, cached text, no adapters/prefix/custom cohort")
    ...
    if args.native_review is not None:
        ...
        require(previous["complete"] and previous["model"] == "juggernaut-x-v10" and previous["revision"] == REVISION
                and previous["native_resolution"] == 1024 and previous["steps"] == 4,
                "512 review must refer to this completed native1024 base protocol")
    require((args.steps, args.guidance_scale) in ((35,3.0),), "Use frozen35steps/CFG3 quality arm")
    require(args.resolution == 1024 or args.steps == 4, "512 arm frozen to4/CFG1")
    if args.cached_text:
        require(sha(COMPONENTS / "provenance.json") == COMPONENT_PROVENANCE_SHA256, "Verified component export changed")
        exported = json.loads((COMPONENTS / "provenance.json").read_text())
        require(exported["complete"] and exported["upstream_checkpoint_sha256"] == CHECKPOINT_SHA256
                and all(v["equal"] for v in exported["verifications"]), "Component conversion is not verified")
    protocol = {"protocol_id": "juggernaut-full-v10-quality-reference-v1",
                "model": "juggernaut-x-v10", "revision": REVISION, "model_provenance_sha256": PROVENANCE_SHA256,
                ...
                "scheduler": "DPMSolverMultistepScheduler; dpmsolver++, order2, midpoint, Karras, lower_order_final=True", "eta": None,
                "scheduler_overrides": QUALITY_SCHEDULER,
                "author_reference": "30–40steps,CFG3–7,DPM++2M Karras,portrait832x1216/landscape1216x832;35/CFG3 chosen for realism; square1024 is our site-framing choice",
                "eta_basis": "Not applicable: deterministic DPM++2M Karras",
                ...
                "component_provenance_sha256": COMPONENT_PROVENANCE_SHA256 if args.cached_text else None,
                "text_encoders_trained": False if args.cached_text else None,
                "clear_unused_mps_cache_before_vae": args.cached_text,
                ...
                "timing_scope": "CPU RNG/fresh noise + transfer + text encoding or cached-embedding transfer + DPM++2M/UNet + originalVAE + PIL + grayscaleWebP; fixed conditioning preparation excluded in cached mode; excludes other artifact writes, model load, warmup, HTTP, queueing and hypothetical retries",
                "load_strategy": ("Verified FP16 components: CLIPs directly toMPS, save all conditioning onCPU, release CLIPs and clearcache, then UNet/VAE directly toMPS"
                                  if args.cached_text else "Single-file mmap+meta low_cpu_mem_usage FP16 CPU assembly; transfer components toMPS sequentially after return/GC"),
...
def worker(args):
    ...
    if protocol["cached_text"]:
        require(sha(output / "component-provenance.json") == COMPONENT_PROVENANCE_SHA256, "Component snapshot mismatch")
        active_model = COMPONENTS
        active_manifest = json.loads((output / "component-provenance.json").read_text())
    for entry in active_manifest["files"]:
        path = active_model / entry["path"]
    ...
    if protocol["cached_text"]:
        ...
        options = {"variant": "fp16", "torch_dtype": torch.float16, "local_files_only": True,
                   "use_safetensors": True, "low_cpu_mem_usage": True, "device_map": {"": "mps"}}

        def load_component(name, cls):
            begin = time.perf_counter()
            component = cls.from_pretrained(COMPONENTS / name, **options).eval().requires_grad_(False)
            torch.mps.synchronize()
            gc.collect()
            component_timings[name] = time.perf_counter() - begin
            guard(name + "_loaded_direct_mps")
            return component

        encoders = {"text_encoder": load_component("text_encoder", CLIPTextModel),
                    "text_encoder_2": load_component("text_encoder_2", CLIPTextModelWithProjection)}
        pipe = StableDiffusionXLPipeline(**encoders, unet=None, vae=None,
            tokenizer=CLIPTokenizer.from_pretrained(COMPONENTS / "tokenizer", local_files_only=True),
            tokenizer_2=CLIPTokenizer.from_pretrained(COMPONENTS / "tokenizer_2", local_files_only=True),
            scheduler=DPMSolverMultistepScheduler.from_config(json.loads((COMPONENTS / "scheduler/scheduler_config.json").read_text()), **QUALITY_SCHEDULER),
        ...
        for case in protocol["cases"]:
            ...
            for name, tokenizer in (("tokenizer", pipe.tokenizer), ("tokenizer_2", pipe.tokenizer_2)):
                ...
                require(len(ids) <= tokenizer.model_max_length, "Cached prompt would be truncated")
                tokenization[name] = {"token_count": len(ids), "input_ids": ids}
            ...
            reloaded_values = load_file(str(path), device="cpu")
            require(values.keys() == reloaded_values.keys() and all(
                values[name].dtype == reloaded_values[name].dtype and torch.equal(values[name], reloaded_values[name])
                for name in values), "Conditioning tensors changed on CPU save/reload")
            cached_embeddings[case["index"]] = reloaded_values
            cache_records.append({"index": case["index"], "prompt": case["prompt"], "file": path.name,
                "file_sha256": sha(path), "tokenization": tokenization, "all_tensor_values_equal_on_reload": True,
                "tensors": {name: {"shape": list(t.shape), "dtype": str(t.dtype),
                    "value_sha256": hashlib.sha256(t.numpy().tobytes()).hexdigest()} for name, t in values.items()}})
            del encoded
        ...
        write(cache_dir / "manifest.json", {"complete": True, "seconds": conditioning_seconds,
            "component_provenance_sha256": COMPONENT_PROVENANCE_SHA256, "text_encoders_trained": False,
            "negative_conditioning": "Zero embeddings for empty negative prompt when CFG enabled, matching original pipeline policy",
            "timing_note": "Fixed prompt preparation excluded from image time; CPU-to-MPS embedding transfer included",
            "records": cache_records})
        cache_manifest_sha256 = sha(cache_dir / "manifest.json")
        guard("text_encoders_released_before_unet_load")
        pipe.register_modules(unet=load_component("unet", UNet2DConditionModel))
        pipe.register_modules(vae=load_component("vae", AutoencoderKL))
        require(pipe.vae_scale_factor == 2 ** (len(pipe.vae.config.block_out_channels) - 1), "Unexpected VAE factor")
        pipe.default_sample_size = pipe.unet.config.sample_size
    else:
        # Preserve the original uncached single-file path for historical comparisons.
        ...
        del component
    if protocol["cached_text"] and protocol["lora"]:
        from safetensors import safe_open
        adapter = Path(protocol["lora"]["path"])
        require(sha(adapter) == protocol["lora"]["sha256"], "Adapter changed after protocol freeze")
        with safe_open(adapter, framework="pt", device="cpu") as weights:
            keys = list(weights.keys())
        require(keys and all(k.startswith("unet.") for k in keys), "Staged cache supports only UNet adapters; CLIPs must remain frozen")
        pipe.load_lora_weights(str(adapter.parent), weight_name=adapter.name)
        guard("unet_only_lora_loaded_after_conditioning")
    ...
    pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config, **QUALITY_SCHEDULER)
    pipe.set_progress_bar_config(disable=True)
    require(pipe.scheduler.config.prediction_type == "epsilon", "Unexpected DPM++2M prediction type")
    torch.mps.synchronize()
    ...
    runtime = {"python": sys.version, "executable": sys.executable, "platform": platform.platform(),
               ...
               "prompt_embeddings_cached": protocol["cached_text"], "conditioning_seconds": conditioning_seconds,
               "conditioning_manifest_sha256": cache_manifest_sha256,
               "component_provenance_sha256": protocol["component_provenance_sha256"],
               "load_strategy": protocol["load_strategy"], "text_encoders_trained": protocol["text_encoders_trained"],
               "clear_unused_mps_cache_before_vae": protocol["clear_unused_mps_cache_before_vae"],
               "watermark": False, "eta": None,
               ...
               "source_hashes": {"single_file": sha(single_file.__file__), "single_file_model": sha(single_file_model.__file__),
                                 ...
                                 "sdxl_pipeline": sha(pipeline_stable_diffusion_xl.__file__),
                                 "dpm_2m_scheduler": sha(scheduling_dpmsolver_multistep.__file__)},
    ...
    try:
        for case in protocol["cases"]:
            ...
            directory.mkdir()
            record = {**case, "status": "started", "steps": protocol["steps"], "eta": None, "guidance_scale": protocol["guidance_scale"],
            ...
            try:
                ...
                def step_guard(pipeline, step, timestep, values):
                    ...
                    if protocol["clear_unused_mps_cache_before_vae"] and step == len(pipeline.scheduler.timesteps) - 1:
                        gc.collect()
                        torch.mps.empty_cache()
                ...
                with torch.inference_mode():
                    ...
                    pixels = pipe(**prompt_args, height=args.resolution, width=args.resolution,
                                  num_inference_steps=protocol["steps"], guidance_scale=protocol["guidance_scale"], negative_prompt=None,
                ...
                require(counter["calls"] == protocol["steps"], "DPM++2M did not execute expected UNet calls; raw images retained")
    ...
    write(output / "result.json", {"complete": True, "model": "juggernaut-x-v10", "revision": REVISION,
          "steps": protocol["steps"], "eta": None, "guidance_scale": protocol["guidance_scale"], "native_resolution": args.resolution,
          ...
          "source_sha256": sha(output / "compare_juggernaut_full.py"), "model_provenance_sha256": PROVENANCE_SHA256,
...
def main():
    ...
    parser.add_argument("--steps", type=int, choices=(35,), default=35)
    parser.add_argument("--guidance-scale", type=float, choices=(3.0,), default=3.0)
    parser.add_argument("--resolution", type=int, choices=(1024,), default=1024)
    ...
    parser.add_argument("--cached-text", action="store_true", default=True)
    ...
    shutil.copy2(Path(__file__), args.output / "compare_juggernaut_full.py")
    shutil.copy2(MODEL / "provenance.json", args.output / "model-provenance.json")
    if args.cached_text:
        shutil.copy2(COMPONENTS / "provenance.json", args.output / "component-provenance.json")
    if args.case_manifest:
