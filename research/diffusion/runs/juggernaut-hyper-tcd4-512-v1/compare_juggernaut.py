...
def preflight(args):
    ...
    protocol = {"protocol_id": "juggernaut-hyper-native-prior-v1" if args.resolution == 1024 else "juggernaut-hyper-512-tradeoff-v1",
                ...
                "resolution": args.resolution, "steps": 4, "guidance_scale": 1.0, "negative_prompt": None,
                ...
                "timing_scope": "CPU RNG creation/fresh noise + transfer + uncached text encoding + TCD/UNet + originalVAE + PIL + grayscaleWebP encoding; excludes artifact hashing/writes except WebP, model load, warmup, HTTP, queueing and hypothetical retries",
...
def worker(args):
    ...
    try:
        for case in protocol["cases"]:
            ...
            directory.mkdir()
            record = {**case, "status": "started", "steps": 4, "eta": .3, "guidance_scale": 1.0,
            ...
            try:
                ...
                with torch.inference_mode():
                    pixels = pipe(prompt=case["prompt"], height=args.resolution, width=args.resolution,
                                  num_inference_steps=4, guidance_scale=1.0, negative_prompt=None, eta=.3,
                ...
                require(counter["calls"] == 4, "TCD4 did not execute exactly four UNet calls; raw images retained")
                require(not pipe.do_classifier_free_guidance, "CFG1 unexpectedly enabled unconditional branch")
    ...
    write(output / "result.json", {"complete": True, "model": "juggernaut-x-hyper", "revision": REVISION,
          "steps": 4, "eta": .3, "guidance_scale": 1.0, "native_resolution": args.resolution,
