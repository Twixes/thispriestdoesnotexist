...
def initialize(args, accelerator):
    ...
    if args.max_train_steps not in (20,100,250) or args.gradient_accumulation_steps != 1:
        raise ValueError('Only bounded20/100/250update runs with accumulation1 are supported')
    if args.rank not in (8,16) or args.train_batch_size != 1 or args.dataloader_num_workers != 0:
        raise ValueError('Require rank8/16, batch1 and no loader subprocesses')
    if args.resume_from_checkpoint or args.push_to_hub or args.train_text_encoder:
        raise ValueError('No resume, publication or text encoder training')
    if args.validation_prompt or args.checkpoints_total_limit is not None:
        raise ValueError('Separate inference required; all checkpoints must be retained')
    if args.mixed_precision != 'fp16' or not args.gradient_checkpointing:
        raise ValueError('Require FP16 frozen weights and gradient checkpointing')
    if args.use_8bit_adam or args.enable_xformers_memory_efficient_attention or args.enable_npu_flash_attention:
        raise ValueError('Unsupported quantization or attention backend')
    if args.random_flip or not args.center_crop or args.resolution != 512 or args.max_train_samples or args.image_interpolation_mode not in ('lanczos','bilinear'):
        raise ValueError('Cache requires512 center crop, no random flip or train subset')
    if args.snr_gamma is not None or args.noise_offset or args.prediction_type is not None:
        raise ValueError('Use unchanged scheduler prediction type and plain upstream noise MSE')
    if args.seed is None or args.checkpointing_steps <= 0:
        raise ValueError('Explicit seed and positive checkpoint interval required')
    if args.pretrained_vae_model_name_or_path:
        raise ValueError('Cache is bound to the original Turbo VAE')
    state = {'output': Path(args.output_dir), 'started': time.monotonic(),
             'initial_swap_used': psutil.swap_memory().used, 'initial_base': None,
             'initial_adapter': None, 'steps': 0, 'expected_steps':args.max_train_steps}
    state['output'].mkdir(parents=True, exist_ok=True)
    for name in ('run_training.py', 'guards.py', 'priest_cache.py', 'patch-provenance.json'):
...
def resources(state, phase, step, **extra):
    ...
    if entry['rss_bytes'] > 24*1024**3:
        ...
        reason = 'system available memory below20%'
...
def finish_evidence(state, model):
    ...
    if state['steps'] != state['expected_steps']:
        raise RuntimeError('Incomplete bounded run')
...
def saved_adapter_evidence(directory, expected_keys):
    ...
    expected = {'unet.'+name for name in expected_keys}
    if set(records) != expected:
        raise RuntimeError('Serialized adapter keys differ from expected converted Diffusers state')
    with path.open('rb') as stream:
        digest = hashlib.file_digest(stream, 'sha256').hexdigest()
    write_json(directory/'saved-adapter-evidence.json', {'path': path.name, 'sha256': digest,
        'keys_exactly_match_converted_diffusers_state': True, 'tensors': records, 'production_approved': False})
