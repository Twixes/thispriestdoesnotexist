...
def worker(args):
    ...
    for item in provenance["files"]:
        if sha(model / item["path"]) != item["sha256"]:
            raise ValueError(f"Weight/config hash mismatch: {item['path']}")
    ...
    for index in range(-1, 8):
        ...
        latent = torch.randn((1, 4, 64, 64), generator=rng)
        ...
        image = pipe(prompt=prompt, height=512, width=512, num_inference_steps=args.steps, guidance_scale=0.0,
    ...
    for i in range(8):
        with Image.open(args.output / f"{i:03}/display.webp") as im:
            contact.paste(im, ((i % 4) * 512, (i // 4) * 512))
    ...
    write(args.output / "result.json", {"complete": True, "model": args.model, "revision": provenance["revision"],
          "steps": args.steps, "native_resolution": 512, "load_seconds": load_seconds, "device": "Apple M5 Pro MPS",
          "server_latency_proven": False, "post_trained": False, "all_outputs_included": True, "records": records})


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", choices=["sd-turbo", "sdxl-turbo"], required=True)
    ...
    write(args.output / "protocol.json", {"model": args.model, "steps": args.steps, "count": 8, "prompts": PROMPTS,
          "seed_base": 202609214000, "warmup_seed": 202609213999, "guidance_scale": 0, "resolution": 512,
    ...
    with (args.output / "worker.log").open("w") as log:
        process = subprocess.Popen([sys.executable, str(Path(__file__)), "--model", args.model, "--steps", str(args.steps), "--output", str(args.output), "--worker"], stdout=log, stderr=subprocess.STDOUT, start_new_session=True)
        try:
