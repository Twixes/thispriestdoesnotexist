...
Optional --cached-prompts encodes the fixed prompts before image-model loading, saves
the FP16 conditioning, then releases both text encoders. Generation timings in that
mode exclude startup text encoding but include cached-embedding device transfer.
"""
import argparse
import gc
import hashlib
...
def worker(args):
    ...
    prompt_cache_manifest = None
    if args.cached_prompts:
        # The original encode_prompt path accepts absent VAE/UNet when CLIP is present.
        encoder_pipe = StableDiffusionXLPipeline(
            vae=None, text_encoder=text_encoder, text_encoder_2=text_encoder_2,
            tokenizer=tokenizer, tokenizer_2=tokenizer_2, unet=None, scheduler=None,
            force_zeros_for_empty_prompt=True, add_watermarker=False)
        text_encoder.requires_grad_(False)
        text_encoder_2.requires_grad_(False)
        cache_dir = output / "prompt-cache"
        cache_dir.mkdir()
        cache_entries = []
        cache_started = time.perf_counter()
        for case, _, _, _ in prepared:
            for arm in arms:
                prompt = arm_prompt(case, arm)
                for tok in (tokenizer, tokenizer_2):
                    require(len(tok(prompt, truncation=False)["input_ids"]) <= tok.model_max_length,
                            "Frozen prompt unexpectedly exceeds tokenizer context")
                with torch.inference_mode():
                    embeds, negative, pooled, negative_pooled = encoder_pipe.encode_prompt(
                        prompt=prompt, device="mps", num_images_per_prompt=1,
                        do_classifier_free_guidance=False, negative_prompt=None)
                    require(negative is None and negative_pooled is None, "Unexpected CFG cache")
                    values = {"prompt_embeds": embeds.to(device="cpu").contiguous(),
                              "pooled_prompt_embeds": pooled.to(device="cpu").contiguous()}
                expected_shapes = {"prompt_embeds": (1, 77, 2048), "pooled_prompt_embeds": (1, 1280)}
                tensor_evidence = {}
                for name, value in values.items():
                    require(value.dtype == torch.float16 and tuple(value.shape) == expected_shapes[name],
                            "Unexpected cached conditioning dtype/shape")
                    require(bool(torch.isfinite(value).all()), "Nonfinite cached conditioning")
                    tensor_evidence[name] = {"shape": list(value.shape), "dtype": str(value.dtype),
                        "values_sha256": hashlib.sha256(value.numpy().tobytes()).hexdigest()}
                path = cache_dir / (case["case_id"] + "-" + arm["id"] + ".safetensors")
                save_file(values, path)
                entry = {"case_id": case["case_id"], "arm": arm["id"],
                         "prompt_variant": "trigger" if arm["prompt_transform"] != "identity" else "plain",
                         "prompt": prompt, "prompt_sha256": hashlib.sha256(prompt.encode()).hexdigest(),
                         "path": str(path.relative_to(output)), "sha256": sha(path),
                         "bytes": path.stat().st_size, "tensors": tensor_evidence}
                cache_entries.append(entry)
                write(path.with_suffix(".json"), entry)
                prompt_cache[(case["case_id"], arm["id"])] = {"values": values, "evidence": entry}
                del embeds, pooled, negative, negative_pooled
                guard(case["case_id"] + "/" + arm["id"] + "/prompt_cached")
        torch.mps.synchronize()
        prompt_cache_seconds = time.perf_counter() - cache_started
        prompt_cache_manifest = {"complete": True, "count": len(cache_entries),
            "protocol_sha256": PROTOCOL_SHA256, "model_provenance_sha256": protocol["base"]["provenance_sha256"],
            "dtype": "float16", "encoding_seconds_including_serialization_and_guards": prompt_cache_seconds,
            "numeric_equivalence": "Same FP16 encode_prompt inputs; equivalence to uncached generation is a hypothesis, not a measured guarantee. Compare saved tensor hashes across matching prompt variants.",
            "entries": cache_entries}
        write(cache_dir / "manifest.json", prompt_cache_manifest)
        del encoder_pipe, text_encoder, text_encoder_2
        text_encoder = text_encoder_2 = None
        gc.collect()
        torch.mps.empty_cache()
        guard("cached_text_encoders_released_before_image_models")
    ...
    runtime = {"python": sys.version, "executable": sys.executable, "platform": platform.platform(),
               ...
               "cached_prompts": args.cached_prompts, "prompt_cache_startup_seconds": prompt_cache_seconds,
               "model_load_seconds_includes_prompt_cache_startup": args.cached_prompts,
               "prompt_cache_manifest_sha256": sha(output / "prompt-cache/manifest.json") if args.cached_prompts else None,
               "conditioning_timing_scope": "Cached CPU-to-MPS conditioning transfer included; startup CLIP encoding excluded" if args.cached_prompts else "Original per-image CLIP encoding included",
    ...
    for case, noise, state, randomness in prepared:
        ...
        for arm in arms:
            ...
            record = {"case_id": case["case_id"], "seed": case["seed"], "arm": arm["id"], "prompt": prompt,
                      ...
                      "cached_prompts": args.cached_prompts,
                      "cached_conditioning": prompt_cache[(case["case_id"], arm["id"])]["evidence"] if args.cached_prompts else None,
            ...
            try:
                ...
                with torch.inference_mode():
                    ...
                    if args.cached_prompts:
                        conditioning = {name: value.to(device="mps") for name, value in
                                        prompt_cache[(case["case_id"], arm["id"])]["values"].items()}
                    pixels = pipe(**conditioning, height=512, width=512, num_inference_steps=args.steps,
    ...
    write(output / "result.json", {"complete": True, "protocol_sha256": PROTOCOL_SHA256, "arms": args.arms,
          ...
          "cached_prompts": args.cached_prompts,
          "conditioning_timing_scope": runtime["conditioning_timing_scope"],
          "prompt_cache_manifest_sha256": runtime["prompt_cache_manifest_sha256"],
...
def main():
    ...
    parser.add_argument("--cached-prompts", action="store_true",
                        help="Cache FP16 prompt embeddings at startup and release CLIP before image-model loading")
