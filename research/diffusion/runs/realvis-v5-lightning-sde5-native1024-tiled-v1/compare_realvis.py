"""Uncurated offline RealVisXL V5 Lightning prior comparison: stochastic DPM++SDE Karras5, CFG1.
...
PROVENANCE_SHA256 = "f9c32b593d32f2461d11a0149473c0a4cf73c808a4303b87a958b07cff9a95a1"
...
LIMIT_SECONDS = 1200
GIB = 1024 ** 3
SAMPLER_CONFIG = {"algorithm_type": "sde-dpmsolver++", "solver_order": 2, "solver_type": "midpoint",
                  "lower_order_final": True, "use_karras_sigmas": True, "use_exponential_sigmas": False,
                  "use_beta_sigmas": False, "final_sigmas_type": "zero", "prediction_type": "epsilon"}
...
def preflight(args):
    ...
    if args.native_review is not None:
        ...
        require(previous["complete"] and previous["model"] == "realvisxl-v5-lightning" and previous["revision"] == REVISION
                and previous["native_resolution"] == 1024 and previous["steps"] == 5,
                "512 review must refer to this completed native1024 base protocol")
    protocol = {"protocol_id": "realvisxl-v5-lightning-native-prior-v1" if args.resolution == 1024 else "realvisxl-v5-lightning-512-tradeoff-v1",
                "model": "realvisxl-v5-lightning", "revision": REVISION, "model_provenance_sha256": PROVENANCE_SHA256,
                "original_comparison_result_sha256": ORIGINAL_RESULTS_SHA256,
                "resolution": args.resolution, "steps": 5, "guidance_scale": 1.0, "negative_prompt": None,
                "scheduler": "DPMSolverSinglestepScheduler", "sampler_config": SAMPLER_CONFIG,
                "sampler_basis": "Author DPM++SDE/Karras recommendation; HF scheduler mapping; explicit stochastic algorithm avoids default ODE. Not a bit-identical A1111 Brownian claim.",
                "native_reference_resolution": 1024, "non_native_tradeoff": args.resolution == 512,
                "cases": cases, "count": 8, "warmup_count": 1, "all_outputs_retained": True,
                ...
                "timing_scope": "CPU RNG creation/fresh noise + transfer + uncached text encoding + DPM++SDE/UNet + originalVAE + PIL + grayscaleWebP encoding; excludes artifact hashing/writes except WebP, model load, warmup, HTTP, queueing and hypothetical retries",
                "load_strategy": "FP16 Diffusers components loaded sequentially directly toMPS using low_cpu_mem_usage; no Turbo components/cache",
                "vae_tiling": {"enabled": args.tiled_vae, "sample_tile": 512, "latent_tile": 64, "overlap": .25},
                "post_trained": False, "production_approved": False, "server_latency_proven": False,
...
def worker(args):
    ...
    from diffusers import StableDiffusionXLPipeline, DPMSolverSinglestepScheduler, UNet2DConditionModel, AutoencoderKL
    from transformers import CLIPTextModel, CLIPTextModelWithProjection, CLIPTokenizer
    from diffusers.models import model_loading_utils
    from diffusers.schedulers import scheduling_dpmsolver_singlestep
    ...
    for entry in manifest["files"]:
        ...
        require(path.is_file(), "Missing local file; run diffusion/restore.py for this model separately: " + str(path))
    ...
    component_timings = {}
    components = {}
    for name, cls in (("text_encoder", CLIPTextModel), ("text_encoder_2", CLIPTextModelWithProjection),
                      ("unet", UNet2DConditionModel), ("vae", AutoencoderKL)):
        begin = time.perf_counter()
        component = cls.from_pretrained(MODEL / name, **options).eval()
    ...
    pipe = StableDiffusionXLPipeline(**components,
        ...
        scheduler=DPMSolverSinglestepScheduler.from_pretrained(MODEL / "scheduler", local_files_only=True, **SAMPLER_CONFIG),
    ...
    require(pipe.scheduler.config.prediction_type == "epsilon", "Unexpected stochastic DPM++SDE prediction type")
    torch.mps.synchronize()
    ...
    runtime = {"python": sys.version, "executable": sys.executable, "platform": platform.platform(),
               ...
               "component_transfer_seconds": component_timings, "scheduler": dict(pipe.scheduler.config), "sampler_label": "DPM++SDE Karras via explicit stochastic Diffusers singlestep",
               ...
               "prompt_embeddings_cached": False, "watermark": False,
               "vae_tiling": protocol["vae_tiling"],
               "source_hashes": {"model_loading_utils": sha(model_loading_utils.__file__),
                                 "dpm_singlestep_scheduler": sha(scheduling_dpmsolver_singlestep.__file__)},
    ...
    try:
        for case in protocol["cases"]:
            ...
            directory.mkdir()
            record = {**case, "status": "started", "steps": protocol["steps"], "guidance_scale": protocol["guidance_scale"],
            ...
            try:
                ...
                with torch.inference_mode():
                    pixels = pipe(prompt=case["prompt"], height=args.resolution, width=args.resolution,
                ...
                record.update(status="complete", noise_generation_seconds=noise_seconds, pipeline_seconds=pipeline_seconds,
                              ...
                              actual_cfg_enabled=pipe.do_classifier_free_guidance, prompt_embedding_cache_used=False)
                require(counter["calls"] == len(pipe.scheduler.timesteps), "UNet count differs from scheduler timestep count; raw images retained")
        ...
        handle.remove()
    require(len(records) == 9, "Expected all8 plus warmup")
    ...
    write(output / "result.json", {"complete": True, "model": "realvisxl-v5-lightning", "revision": REVISION,
          "steps": protocol["steps"], "guidance_scale": protocol["guidance_scale"], "native_resolution": args.resolution,
          "device": "Apple MPS", "load_seconds": load_seconds, "scheduler": dict(pipe.scheduler.config), "sampler_label": "DPM++SDE Karras via explicit stochastic Diffusers singlestep",
          "runtime_sha256": sha(output / "runtime.json"), "protocol_sha256": sha(output / "protocol.json"),
          "source_sha256": sha(output / "compare_realvis.py"), "model_provenance_sha256": PROVENANCE_SHA256,
          "contact_sha256": sha(output / "contact.png"), "warm_timings": timings,
          "all_outputs_included": True, "count": 8, "warmup_retained": True, "post_trained": False,
...
def main():
    ...
    shutil.copy2(Path(__file__), args.output / "compare_realvis.py")
