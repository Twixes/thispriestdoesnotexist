...
def main():
    ...
    trainer = ROOT / "vendor/flux-train" / ("run_training_cached.py" if args.cache else "run_training.py")
    minimum_available = .20 if args.cache else .25
    ...
    if args.cache:
        command.extend(["--priest_cached_data", str(args.cache.resolve())])
    ...
    write(output / "launch.json", {"command": command, "purpose": "20-update MPS compatibility test",
        ...
        "minimum_available_memory_fraction": minimum_available, "source_sha256": sha(trainer),
        "cache_manifest_sha256": sha(args.cache / "manifest.json") if args.cache else None,
    ...
    with (output / "worker.log").open("w") as log, (output / "resources.jsonl").open("w") as monitor:
        ...
        try:
            while process.poll() is None:
                ...
                elif vm.available / vm.total < minimum_available:
                    failure = f"Available system memory below{minimum_available:.0%}"
                elif elapsed > 1800:
