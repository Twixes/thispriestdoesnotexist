"""Fetch a pinned Juggernaut full v10 checkpoint/config/license; no model execution."""
import argparse
...
WEIGHT = "Juggernaut-X-RunDiffusion-NSFW.safetensors"
WEIGHT_SHA256 = "d91d35736d8f2be038f760a9b0009a771ecf0a417e9b38c244a84ea4cb9c0c45"
WEIGHT_BYTES = 7105348672
...
def main():
    ...
    manifest = {"complete": True, "created_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
          ...
          "checkpoint_kind": "publisher single-file complete SDXL full non-Hyper checkpoint; no conversion performed",
          ...
          "inference_reference": {"author_steps": [30, 40], "chosen_steps": 35,
              "author_samplers": ["DPM++ 2M Karras"], "author_cfg_range": [3.0, 7.0], "chosen_cfg": 3.0,
              "author_resolutions": [[832, 1216], [1216, 832]],
              "comparison_resolution": [1024, 1024], "resolution_note": "Square SDXL native-pixel-count comparison for site; differs from author portrait/landscape framing",
              "sampler_mapping": "DPMSolverMultistepScheduler algorithm_type=dpmsolver++,solver_order=2,use_karras_sigmas=True; deterministic 2M, not SDE",
              "candidate": "Full non-distilled quality reference35steps,CFG3; not a latency candidate and not more steps on Hyper",
              "lora": "SDXL structure compatibility does not establish transfer quality"},
