"""Fixed, research-only FFHQ source batch with exact service latents and mapped W."""
import hashlib
import io
import json
...
def pixels_difference(left, right):
    delta = np.abs(left.astype(np.int16) - right.astype(np.int16))
    return {'max_absolute_channel_difference': int(delta.max()),
            'mean_absolute_channel_difference': float(delta.mean()),
            'fraction_unequal_channels': float(np.mean(delta != 0)),
            'rmse_channels': float(np.sqrt(np.mean(delta.astype(np.float64) ** 2)))}
...
def generate(name, seed_hex):
    ...
    ws = captured.pop('ws')
    np.savez(latent_path, z=z, w=ws[:, 0, :], ws=ws)
    row = {'id': name, 'seed_hex': seed_hex, 'image': str(path.relative_to(HERE)),
           ...
           'z_shape': list(z.shape), 'w_shape': list(ws[:, 0, :].shape), 'ws_shape': list(ws.shape),
...
for index in range(32):
    seed_hex = hashlib.sha256(f'priest-paired-source-v1:{index}'.encode('utf-8')).hexdigest()
    row, _ = generate(f'{index:03}', seed_hex)
    row['seed_derivation_index'] = index
    rows.append(row)
batch_seconds = time.monotonic() - batch_started
assert len({row['seed_hex'] for row in rows}) == 32
assert len({row['rgb_sha256'] for row in rows}) == 32

original = ROOT / 'research/runs/inference-cpu/ffhq1024/docker-4g/sample-002.webp'
original_manifest = json.loads(original.with_name('benchmark.json').read_text())
original_row = next(row for row in original_manifest['generations'] if row['sample'] == original.name)
assert sha(original) == original_row['sha256']
reproduction, pixels = generate('reproduction-sample002', original_row['seed'])
encoded = io.BytesIO()
Image.fromarray(pixels).save(encoded, format='WEBP', quality=90, method=4)
webp_path = HERE / 'images/reproduction-sample002.webp'
webp_path.write_bytes(encoded.getvalue())
original_rgb = np.asarray(Image.open(original).convert('RGB'))
encoded_rgb = np.asarray(Image.open(io.BytesIO(encoded.getvalue())).convert('RGB'))
reproduction.update({
    'original_path': str(original.relative_to(ROOT)), 'original_sha256': sha(original),
    'encoded_webp_path': str(webp_path.relative_to(HERE)), 'encoded_webp_sha256': sha(webp_path),
    'encoded_webp_byte_exact': encoded.getvalue() == original.read_bytes(),
    'decoded_reencoded_webp_vs_original_webp': pixels_difference(encoded_rgb, original_rgb),
    'native_png_pixels_vs_original_decoded_webp': pixels_difference(pixels, original_rgb),
    'interpretation': 'Native macOS CPU versus original Linux amd64 CPU; backend and WebP encoder may differ. No tolerance pass is asserted.',
})
hook.remove()
contact = Image.new('RGB', (8 * 192, 4 * 214), (24, 24, 24))
...
report = {
    'purpose': 'Offline paired-data source selection; not a production seed catalog',
    'training_eligible': False, 'production_approved': False, 'is_priest_generator': False,
    'count': 32, 'seed_derivation': "SHA256(UTF8('priest-paired-source-v1:' + decimal zero-based index))",
    ...
    'mapping_recipe': 'Capture actual mapping forward output during exact generator forward; w is ws[:,0,:], ws preserves all synthesis inputs',
    ...
    'batch32_seconds': batch_seconds, 'total_seconds': time.monotonic() - started,
    'peak_rss_bytes': resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,
    'contact_sha256': sha(HERE / 'contact.png'), 'entries': rows, 'reproduction': reproduction,
}
(HERE / 'manifest.json').write_text(json.dumps(report, indent=2) + '\n')
print(json.dumps({key: value for key, value in report.items() if key not in ['entries', 'constant_noise_raw_hashes', 'reproduction']}, indent=2), flush=True)
