"""CPU-only geometry audit of one validation-only raw edit; no alignment or image mutation."""
import ast
import hashlib
import json
...
sources_manifest_path = HERE.parent / 'ffhq-paired-sources32/manifest.json'
...
for index, row in enumerate(prompts['records']):
    ...
    entries.append({'id': row['id'], 'training_eligible': False, 'source_rgb': raw,
                    ...
                    'split': 'validation', 'collar_complete_in_frame': False,
                    'collar_review': 'Clear white Roman tab visible, reaching lower frame edge; no requirement for whole collar in frame'})
    ...
    hashes.append({'id': row['id'], 'source_path': row['source'], 'source_sha256': sha(source_path),
                   ...
                   'training_eligible': False, 'split': 'validation', 'included_in_training_proposal': False})
canvas.save(HERE / 'comparison.png')
result = {'purpose': 'One built-in imagegen clothing edit reserved strictly for held-out validation, not a trained-priest-model output',
