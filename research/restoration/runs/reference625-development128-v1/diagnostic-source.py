...
def cohort_inputs(cohort, supplied_sha, manifest=None):
    if cohort=='manifest256':
        require(manifest is not None, 'Need explicit completed generation manifest')
        m=read(manifest)
        require(m['complete'] is True and m['resolution']==256, 'Expected complete256 generation')
        require(1 <= len(m['entries']) <= 128, 'Bound manifest to128 attempts')
        rows=[]
        for i,x in enumerate(m['entries']):
            require(x['index']==i, 'Manifest must retain every sequential attempt')
            path=(manifest.parent/x['path']).resolve()
            require(path.is_relative_to(ROOT/'research/runs'), 'Input outside research runs')
            rows.append({'index':i,'path':str(path.relative_to(ROOT)),'sha256':x['sha256'],'raw_challenge_control':None})
        return rows
...
def worker(args):
    ...
    helper=FaceRestoreHelper(upscale_factor=1 if args.cohort=='nada500' else 4,face_size=512,crop_ratio=(1,1),det_model='retinaface_resnet50',save_ext='png',pad_blur=False,use_parse=True,device=torch.device('cpu'),model_rootpath=str(BASE/'weights'))
    ...
    for item in inputs:
        ...
        with Image.open(folder/'raw.png') as im:
            expected=1024 if args.cohort=='nada500' else 256
...
def supervise(args):
    ...
    process_guard();mem=memory_guard();pins=pins_check();inputs=cohort_inputs(args.cohort,args.nada_checkpoint_sha256,args.manifest)
    for x in inputs:require(sha(ROOT/x['path'])==x['sha256'],'Input mismatch')
    args.output.mkdir(parents=True)
    write(args.output/'inputs.json',inputs)
    shutil.copyfile(Path(__file__),args.output/'diagnostic-source.py')
    if args.manifest:
        shutil.copyfile(args.manifest,args.output/'generation-manifest.json')
...
def main():
    ...
    parser.add_argument('--cohort',choices=['reference625','nada500','manifest256'],required=True)
