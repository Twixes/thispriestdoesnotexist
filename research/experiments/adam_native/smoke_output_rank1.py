"""Prepared ten-iteration native1024 OUTPUT-RANK1 compatibility experiment.
...
def prepare(out):
    ...
    assert sha(HERE/'smoke.py') == 'a0f58197d99dea71e19f6b1ce0ade2491173bba1576c8e4ba44ed844faf48e63'
    assert sha(HERE/'modulation.py') == 'b032f7965f397a5ebdc93fb1081adaeace651d31a614347123ab83fc46ce456a'
    sources = [HERE/'smoke.py', HERE/'smoke_output_rank1.py', HERE/'modulation.py', HERE/'upstream/provenance.json', GDIR/'model.json', DDIR/'metadata.json']
    ...
    protocol = {'name':'adam-native1024-output-rank1-smoke10', 'conv_layout':'output_rank1', 'iterations':10, 'resolution':1024,
        ...
        'deviations':['Paper-aligned output_rank1 convolution outer(v_out,u_flat), differing from released source_flattened control',
    ...
    shutil.copy2(HERE/'smoke_output_rank1.py',out/'smoke_output_rank1.py')
...
def supervise(out):
    ...
    try:
        with (out/'worker.log').open('w') as log:
            proc = subprocess.Popen([sys.executable, str(HERE/'smoke_output_rank1.py'),'--worker','--output',str(out)],
...
def worker(out):
    ...
    inventories={'G':install_modulation(G,component='G',conv_layout=protocol['conv_layout']),'D':install_modulation(D,component='D',conv_layout=protocol['conv_layout'])}
