"""Prepared, supervised EMA loss-gradient importance measurement; no optimization.
...
def prepare(run, step, pairs, out):
    import psutil
    ...
    for path in (HERE / 'estimate_importance.py', HERE / 'importance.py',
    ...
    protocol = {
        'name': 'adam-output-rank1-ema-importance', 'pairs': pairs, 'seed': SEED,
        ...
        'resolution': 1024, 'device': 'cpu', 'threads': 1,
        ...
        'parameter_scope': 'All u/v/b modulation coordinates; no ordinary base-weight or D epilogue gradients are stored. Their partial derivatives are unused by factor-score selection.',
    ...
    for name in ('estimate_importance.py', 'importance.py', 'modulation.py'):
        shutil.copy2(HERE / name, out / name)
    return protocol


def supervise(run, step, pairs, out):
    import psutil
    protocol = prepare(run, step, pairs, out)
    start = time.monotonic(); swap = psutil.swap_memory().used
    proc = None; failure = None; peak = 0; min_available = 1.0
    try:
        with (out / 'worker.log').open('w') as log:
            proc = subprocess.Popen([sys.executable, str(HERE / 'estimate_importance.py'),
...
def worker(out):
    ...
    write(out / 'result.json', {'complete': True, 'pairs': protocol['pairs'],
        'seconds': time.monotonic() - start, 'ema_state_unchanged': True, 'ema_state_sha256': after,
...
if __name__ == '__main__':
    ...
    if args.worker:
        ...
        supervise(args.run.resolve(), args.step, args.pairs, out)
