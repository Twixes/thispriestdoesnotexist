"""Evaluate a completed research student on disjoint saved source latents, CPU only."""
import argparse
import gc
import hashlib
import json
...
def verify_metadata(metadata, checkpoint, configuration, source_manifest, weights_sha, metadata_sha):
    if checkpoint['format'] != 'paired-edit-research-v1' or checkpoint['production_approved'] is not False:
        raise ValueError('Expected an explicitly unapproved paired research checkpoint')
...
def region_errors(source_gray, student_gray):
    """Unclipped luminance MAE; fixed horizontal regions are not face/clothing masks."""
    if source_gray.shape != student_gray.shape or source_gray.ndim != 2:
        raise ValueError('Expected equally sized grayscale planes')
    boundary = int(source_gray.shape[0] * .75)
    if boundary == 0 or boundary == source_gray.shape[0]:
        raise ValueError('Both horizontal regions must be nonempty')
    delta = np.abs(source_gray.astype(np.float64) - student_gray.astype(np.float64))
    return {'full_mae_0_1': float(delta.mean()), 'top75_mae_0_1': float(delta[:boundary].mean()),
            'bottom25_mae_0_1': float(delta[boundary:].mean()), 'max_absolute_difference_0_1': float(delta.max())}
...
def arguments():
    ...
    parser.add_argument('--sources', required=True, type=Path, help='Fixed source batch manifest; reproduction entry is excluded')
...
def main():
    ...
    from trainer import Generator, freeze_student, frozen_state, state_digest, grayscale, quantized
    ...
    for name, expected in checkpoint['provenance']['source_files'].items():
        path = HERE / 'trainer.py' if name == 'trainer' else vendor / name
        if sha256(path) != expected:
            raise ValueError(f'Checkpoint code provenance differs: {name}')
    ...
    report = {'purpose': 'Research-only disjoint-latent source/student comparison', 'production_approved': False,
              ...
              'script_sha256': sha256(__file__), 'contact_sha256': sha256(args.output / 'comparison.png'),
