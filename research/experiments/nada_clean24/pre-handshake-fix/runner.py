"""Guarded, explicit native1024 NADA-style ten-update smoke; stdlib supervisor."""
import argparse
import hashlib
...
def concurrency_guard():
    ...
    for line in processes.splitlines():
        ...
        if pid.isdigit() and int(pid) != os.getpid() and re.search(r'(^|/|\s)python[\w.]*\s', command):
            if 'research/' in command and any(word in command for word in
                ('trainer.py', 'train.py', 'runner.py', 'evaluate', 'benchmark', 'recover-factorial', 'verify-checkpoint')):
...
def supervise(output):
    ...
    launch = {'started_unix': started, 'deadline_unix': deadline, 'nonce': nonce,
              ...
              'numeric_environment': NUMERIC_ENV, 'maximum_updates': 10,
    ...
    try:
        with (output / 'worker.log').open('x') as stream:
            ...
            write_new(output / 'child.json', {'pid': process.pid, 'command': command})
...
def main():
    ...
    if args.check_only:
        if args._worker or args._deadline is not None or args._nonce is not None:
