...
def equivalent_pipeline(a,b):
    # Python function repr embeds a process address, not a preprocessing parameter.
    # Keep every other source hash, transform parameter and model identity exact.
    def canonical(p):
        q=dict(p)
        q['preprocess']=re.sub(r'(<function _convert_image_to_rgb) at 0x[0-9a-f]+>', r'\1>',q['preprocess'])
        return q
...
def main(args):
    ...
    if args.mode=='fit':
        ...
        require(model['complete'] and model['mode']=='fit' and model['protocol_sha256']==protocol_sha and equivalent_pipeline(model['pipeline'],pipeline),'Frozen model/protocol/feature pipeline mismatch')
