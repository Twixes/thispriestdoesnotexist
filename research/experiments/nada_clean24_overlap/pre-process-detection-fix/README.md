...
Root observed reference PID27880 at62–64% free system memory across steps528–566 despite changing RSS. The read-only preparation check here observed60% free and matched the exact current reference process. These are point observations, not bounds on MPS allocations. CPU training can contend with MPS for unified memory/bandwidth and slow both jobs. Fresh execution checks, continuous pressure observations and aborts are required; the unknown NADA peak remains unmeasured.
