"""Guarded, explicit native1024 NADA-style 10-to-500 continuation; stdlib supervisor."""
import argparse
import hashlib
    ...
    def watchdog():
        while not stop.wait(.05):
            ...
            if time.time() >= deadline:
                reason = 'Original sixty-minute absolute deadline expired'
            if reason:
                try:
...
def supervise(output, restore_only=False):
    ...
    launch = {'started_unix': started, 'deadline_unix': deadline, 'nonce': nonce,
              ...
              'numeric_environment': NUMERIC_ENV, 'maximum_updates': 0 if restore_only else 490, 'start_step': 10,
              'end_step': 10 if restore_only else 500, 'restore_only': restore_only,
    ...
    try:
        with (output / 'worker.log').open('x') as stream:
            ...
            while process.poll() is None:
                ...
                if time.time() >= deadline:
                    raise RuntimeError('Sixty-minute absolute deadline expired')
            ...
            expected_updates, expected_step = (0, 10) if restore_only else (490, 500)
