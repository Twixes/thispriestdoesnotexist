...
SOURCE = Path(__file__).resolve().parent / 'launch.py'
...
def scenario(name, directory):
    ...
    phases = directory / 'fake-phases'
    phases.mkdir()
    for filename in ['trainer.py', 'smoke.py']:
        (phases / filename).write_text('# fixture only, never executed\n')
    module.ROOT = directory
    module.OUTPUT = run
    module.PHASES = phases
    module.preflight = lambda: {'fixture': 'No source/model reads required'}
    module.execution_guard = lambda: 'Fixture memory and concurrency check; no model allocation'
    module.MAX_SECONDS = 1.0 if name == 'deadline' else 10
    # The real command is never executed. Replace it before main is called.
    module.command = lambda: ['dummy-command-never-executed']
    real_popen = subprocess.Popen

    def dummy_popen(command, **kwargs):
        assert command == ['dummy-command-never-executed'] and kwargs['start_new_session'] is True
    ...
    if name == 'write_failure':
        ...
        def fail_after_spawn(path, value):
            nonlocal failed
            if 'pid' in value and not failed:
                failed = True
    ...
    for name in ['write_failure', 'sigint', 'sigterm', 'deadline']:
        with tempfile.TemporaryDirectory(prefix='priest-recovery-lifecycle-') as temporary:
            ...
            try:
                ...
                record = json.loads((directory / 'fake-run/launch.json').read_text())
                assert all(absent.values()) and no_group, (name, ids, absent, no_group)
                assert exit_code != 0 and record['complete'] is False
                assert record['child_exit_code'] == -signal.SIGKILL
                assert not (directory / 'fake-run/completed.json').exists()
                assert not (directory / 'fake-run/resume.pt').exists()
                if name == 'sigterm':
                ...
                results.append({'scenario': name, 'passed': True, 'wall_seconds': time.monotonic()-tick,
                    ...
                    'dummy_group_absent': no_group, 'child_reaped_returncode': record['child_exit_code'],
