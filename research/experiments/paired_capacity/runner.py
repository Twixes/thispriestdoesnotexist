"""Research-only 50-update paired capacity forks; imports models only in guarded workers."""
import argparse
import copy
import hashlib
import json
...
UPDATES=50
MILESTONES=(0,25,50)
BRANCHES=('baseline','unfreeze-b32')
MAX_RSS=8*1024**3
...
def worker(branch,output):
    ...
    try:
        ...
        optimizer,transfer=fork_optimizer(student,checkpoint['optimizer'],branch=='unfreeze-b32',trainer,torch,pins['trainable_parameters'])
        ...
        fork={'format':'paired-capacity-fork-v1','production_approved':False,'exact_resume':False,'branch':branch,
              'parent_checkpoint_sha256':pins['parent_checkpoint_sha256'],'parent_step':600,'updates':UPDATES,
              'objective':'unchanged parent equal-area collar/rest/protected/fresh losses',
              'freeze_policy':'parent mapping/noise/buffers/blocks<=32 frozen' if branch=='baseline' else 'only b32 non-noise parameters additionally trainable; mapping/blocks<=16/noise/buffers frozen',
        ...
        def save_milestone(probe_step):
            ...
            state={'format':'paired-capacity-fork-v1','production_approved':False,'probe_step':probe_step,'step':600+probe_step,
        ...
        with (output/'metrics.jsonl').open('w') as log:
            for probe_step in range(1,UPDATES+1):
                ...
                tick=time.monotonic();metrics=trainer.update(student,source,optimizer,train_pairs,options,sampling,preservation,'cpu')
...
def main():
    ...
    write_json(output/'plan.json',{'branches':list(BRANCHES),'updates_each':50,'milestones':list(MILESTONES),'parent_checkpoint_sha256':pins['parent_checkpoint_sha256'],'runner_sha256':sha(__file__),'pins_sha256':sha(PINS),'exact_resume':False,'objective_unchanged':True})
    completed=[]
    for branch in BRANCHES:
        # Separate child processes guarantee all tensors/allocator state are gone
        # before the next branch loads; no concurrent paired-capacity workers.
...
if __name__=='__main__':
    raise SystemExit('UNUSED DRAFT: b32 capacity hypothesis deferred. Do not execute; see paired_surround experiment.')
