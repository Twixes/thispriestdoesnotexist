"""Prepared supervised continuation of a future successful retained250 native10.

No automatic launch/restart. The exact frozen AdaptationEngine is imported;
all source G/D/GEMA/Adam/RNG/path/mask state is restored before global step11.
"""
import argparse
import copy
import json
...
PARENT_RUNNER_SHA = '0a5be1d0f4db4ffe04b1c08e6483855fdf99a78b9be8bf38eab1a39672fec7ba'
START = 10
END = 100
...
def validate_parent(run):
    """Requires actual completed native10 and its authenticated FI/retained250 lineage."""
    run=run.resolve();run.relative_to(RESEARCH/'runs')
    assert sha(HERE/'main_adaptation_fixed_offsets250_10.py')==PARENT_RUNNER_SHA
    protocol=read(run/'protocol.json');result=read(run/'result.json');supervisor=read(run/'supervisor.json')
    marker=read(run/'checkpoint-010.json');protocol_sha=sha(run/'protocol.json')
    assert protocol['name']=='adam-native1024-fixed-offsets-retained250-main-adaptation10'
    assert protocol['iterations']==10 and protocol['expected_optimizer_steps']==expected_steps(10)
    ...
    assert protocol['retained250_lineage']['overall_source_run_complete'] is False
    parent_relative=str((HERE/'main_adaptation_fixed_offsets250_10.py').relative_to(ROOT))
    assert protocol['pins'][parent_relative]==PARENT_RUNNER_SHA
    for relative,expected in protocol['pins'].items():assert sha(under_root(relative))==expected,relative
    assert result['complete'] is True and result['iterations']==10
    assert result['optimizer_steps']==expected_steps(10)
    assert result['fixed_offsets_and_mask_invariants'] is True and result['checkpoint_state_restored_exactly'] is True
    assert result['folded_native_max_errors']=={'raw':[0.0]*4,'ema':[0.0]*4}
    assert result['main_adaptation_run'] is True and result['mechanics_smoke_only'] is True
    assert supervisor['complete'] is True and supervisor['failure'] is None
    assert marker['complete'] is True and marker['step']==10 and marker['model_optimizer_rng_path_state_restored_exactly'] is True
    for record in (result,supervisor,marker):assert record['protocol_sha256']==protocol_sha
    assert sha(run/'checkpoint-010.pt')==marker['checkpoint_sha256']
    equality=read(run/'initial-equality.json')
    assert equality['generator_exact'] is True and equality['generator_cases']==4
    assert equality['discriminator_exact'] is True and equality['discriminator_cases']==1
    assert equality['fresh_optimizer_states_empty'] is True
    for step in (0,10):
        ...
        expected_names={f'{label}-{i:03}.png' for label in ('raw','ema') for i in range(4)}
        assert len(manifest['images'])==8 and {image['path'] for image in manifest['images']}==expected_names
        for image in manifest['images']:assert sha(run/f'step-{step:03}'/image['path'])==image['sha256']
    selections,_=base.validated_selection(under_root(protocol['importance_run']),protocol['quantile'])
...
def prepare(run,out):
    ...
    files=[HERE/'continue_main_adaptation_fixed_offsets250_100.py',HERE/'main_adaptation_fixed_offsets250_10.py']
    files += [run/name for name in ('protocol.json','result.json','supervisor.json','checkpoint-010.json','checkpoint-010.pt','initial-equality.json','runtime.json','modulation-inventory.json','eval-z.npz','trainable-inventory.json')]
    for step in (0,10):files += [run/f'step-{step:03}/manifest.json',*sorted((run/f'step-{step:03}').glob('*.png'))]
    for path in files:pins[str(path.resolve().relative_to(ROOT))]=sha(path)
    protocol=copy.deepcopy(source)
    protocol.update(name='adam-native1024-retained250-main-adaptation10-to100',iterations=END,
        ...
        initialization='Authenticate original G/D references and identical masks, then restore all native10 model/Adam/RNG/path state; no reset enters updates',
        preview_steps=[10,50,100],checkpoint_steps=list(CHECKPOINT_STEPS),expected_optimizer_steps=expected_steps(END),
        metrics_range={'first_completed_step':11,'last_completed_step':100,'additional_iterations':90},
        additional_optimizer_steps={key:expected_steps(END)[key]-expected_steps(START)[key] for key in ('g','d')},
        pins=pins,automatic_restart=False,automatic_launch=False,native10_visual_review_required_before_launch=True,
        numerical_disjoint_stability_review_required_before_launch=True)
    protocol['guards']=dict(source['guards'],seconds=3600)
    protocol['deviations']=[item for item in source['deviations'] if not item.startswith('Exactly10 main-adaptation smoke iterations')]
    protocol['deviations'].append('Exact native10 main-adaptation continuation to100; checkpoint/previews50/100; no convergence or quality approval')
...
def worker(out):
    ...
    payload=torch.load(run/'checkpoint-010.pt',weights_only=True,map_location='cpu',mmap=True)
    ...
    write(out/'restoration.json',{'complete':True,'step':10,'parent_checkpoint_sha256':marker['checkpoint_sha256'],
    ...
    def preview(step):
        ...
        assert len(images)==8
        if step==START:
            assert images==read(run/'step-010/manifest.json')['images'],'Parent10 PNG bytes differ; no update permitted'
    ...
    write(out/'result.json',{'complete':True,'iterations':END,'first_completed_step':11,'additional_iterations':90,
...
def supervise(run,out):
    ...
    try:
        with (out/'worker.log').open('w') as log:
            proc=subprocess.Popen([sys.executable,str(HERE/'continue_main_adaptation_fixed_offsets250_100.py'),'--worker','--output',str(out)],
