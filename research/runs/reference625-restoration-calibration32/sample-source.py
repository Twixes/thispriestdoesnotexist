"""Unfiltered reference625 sampling with declared development/calibration/test split; CPU only, no restoration/selection."""
import argparse
import ast
import hashlib
...
def worker(args):
    ...
    guard(deadline)
    write(args.output/'evaluation.json',{'complete':True,'split':args.split,'development_only':args.split=='development','quality_approved':False,'production_approved':False,
...
def supervise(args):
    ...
    command=[sys.executable,str(Path(__file__)),'--output',str(args.output),'--count',str(args.count),'--seed-base',str(args.seed_base),'--split',args.split,'--worker']
...
if __name__=='__main__':
    ...
    declared=read(HERE/'protocol.json')[args.split]
    require(args.count==declared['count'] and args.seed_base==declared['seed_base'],'Count/seed must match declared split')
