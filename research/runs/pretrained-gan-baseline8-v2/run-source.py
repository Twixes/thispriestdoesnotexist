...
DEPENDENCY = Path(__file__).resolve().parent / 'dependencies/setuptools80'
LIMIT = 12 * 1024**3
MODELS = {
...
def verify():
    ...
    dependency = json.loads((DEPENDENCY.parent / 'provenance.json').read_text())
    for name, expected in dependency['extracted_files'].items():
        if sha(DEPENDENCY / name) != expected:
            raise RuntimeError('Pinned setuptools dependency changed: ' + name)
    return source
...
def worker(output):
    ...
    # Only this worker imports the archived setuptools80 compatibility dependency.
    # Shared research/.venv remains unchanged. Official NVIDIA source is unchanged.
    sys.path.insert(0, str(DEPENDENCY))
    import pkg_resources
    if Path(pkg_resources.__file__).resolve() != DEPENDENCY / 'pkg_resources/__init__.py':
        raise RuntimeError('Wrong pkg_resources dependency')
