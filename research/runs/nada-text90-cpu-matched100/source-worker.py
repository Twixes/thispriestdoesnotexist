"""Exact 10-to-100 text-direction NADA-style CPU probe. Imported only after the supervisor guards."""
import copy
import hashlib
...
def run(root, output, pins, restore_only=False):
    ...
    require(len({row.tobytes() for name in ['source_z', 'previous_train_z', 'train_z', 'eval_z'] for row in latent_arrays[name]}) == 140,
            'Original50/first90 fresh z overlap')
    ...
    recipe = {**parent_recipe, 'continuation': continuation, 'input_pins': pins,
              'updates': 90, 'start_step': 10, 'end_step': 100,
              'first_update': 'Already completed in checkpoint010; no bootstrap repeats',
              'updates_11_to_100': 'Unchanged directional cosine with fixed text guidance direction',
              'text_guidance': manifest['text_guidance'],
    ...
    # Sole objective change: a fixed clothing-specific text contrast replaces image-centroid direction.
    text_spec = manifest['text_guidance']
    source_prompts = [template.format(text_spec['source_class']) for template in text_spec['templates']]
    target_prompts = [template.format(text_spec['target_class']) for template in text_spec['templates']]
    before_text_rng = nested_hash(rng())
    with torch.no_grad():
        text_source = unit(encoder.encode_text(clip.tokenize(source_prompts))).detach()
        text_target = unit(encoder.encode_text(clip.tokenize(target_prompts))).detach()
        text_difference = (text_target - text_source).mean(0, keepdim=True)
        require(float(text_difference.norm()) > 1e-6, 'Degenerate fixed text direction')
        direction = unit(text_difference).detach()
    require(nested_hash(rng()) == before_text_rng, 'Fixed text encoding consumed training RNG')
    require(tuple(direction.shape) == (1, 512), 'Unexpected text embedding shape')
    require(base.state_digest(encoder.state_dict()) == initial_clip_hash, 'Text encoding changed CLIP')
    dump(output / 'text-direction.json', {'source_prompts': source_prompts, 'target_prompts': target_prompts,
        'norm_before_normalization': float(text_difference.norm()), 'unit_direction': direction.tolist(),
        'source_unit_embeddings': text_source.tolist(), 'target_unit_embeddings': text_target.tolist(),
        'direction_nested_sha256': nested_hash(direction), 'encoding_rng_unchanged': True,
        'replaces_image_centroid_direction_only': True, 'text_direction_is_not_quality_approval': True})
    del text_source, text_target, text_difference
    ...
    require(base.state_digest(encoder.state_dict()) == initial_clip_hash, 'Frozen CLIP changed')
    frozen_checks()
    dump(output / 'worker-result.json', {'complete': True, 'updates': 90, 'optimizer_calls': 90, 'restore_only': False,
        'start_step': 10, 'total_step': 100, 'raw_generator_only': True,
        'actual_optimizer_and_rng_reload': True, 'checkpoints': checkpoints,
        'source_and_clip_unchanged': True, 'frozen_student_unchanged': True,
        'snapshot_rng_and_state_checks': [10, 50, 100], 'initial_native_png_matches': 16,
