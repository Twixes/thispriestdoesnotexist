"""Read-only CPU validation of trusted local step-125 training artifacts."""
import hashlib
import json
import math
import re
...
def main():
    ...
    (RUN/'checkpoint-000125-memory-before.txt').write_text(preflight)
    free = int(re.search(r'System-wide memory free percentage: (\d+)%', preflight)[1])
    if free < 20:
        raise SystemExit('Defer: less than 20% memory free; no checkpoint load')
    paths = [RUN/name for name in ['resume.pt','generator-000125.pt','config.json',
             'train-source.py','adapters-source.py','samples-000125-ema.png',
             'samples-000125-raw.png','samples-000125-untruncated.png']]
    ...
    generator = torch.load(RUN/'generator-000125.pt', map_location='cpu', mmap=True, weights_only=True)
    config = json.loads((RUN/'config.json').read_text())
    assert checkpoint['format_version'] == 1
    assert checkpoint['batch_idx'] == generator['step'] == 125
    assert checkpoint['images_seen'] == generator['images_seen'] == 125*64 == 8000
    assert checkpoint['sampler']['consumed'] == 8000
    ...
    result = {
        ...
        'limits':['Numeric finiteness and export equality are not image-quality approval.',
                  ...
                  'Only step125 artifacts are reviewed; the live training process continued unchanged.'],
    }
    (RUN/'checkpoint-000125-validation.json').write_text(json.dumps(result,indent=2)+'\n')
