"""Read-only CPU validation of trusted local step-250 training artifacts."""
import hashlib
import json
import math
import re
...
def main():
    ...
    (RUN/'checkpoint-000250-memory-before.txt').write_text(preflight)
    ...
    paths = [RUN/name for name in ['resume.pt','generator-000250.pt','config.json',
             'train-source.py','adapters-source.py','samples-000250-ema.png',
             'samples-000250-raw.png','samples-000250-untruncated.png']]
    ...
    generator = torch.load(RUN/'generator-000250.pt', map_location='cpu', mmap=True, weights_only=True)
    config = json.loads((RUN/'config.json').read_text())
    assert checkpoint['format_version'] == 1
    assert checkpoint['batch_idx'] == generator['step'] == 250
    assert checkpoint['images_seen'] == generator['images_seen'] == 250*64 == 16000
    assert checkpoint['sampler']['consumed'] == 16000
    ...
    result = {
        ...
        'limits':['Numeric finiteness and export equality are not image-quality approval.',
                  ...
                  'Only step250 artifacts are reviewed; the live training process continued unchanged.'],
    }
    (RUN/'checkpoint-000250-validation.json').write_text(json.dumps(result,indent=2)+'\n')
