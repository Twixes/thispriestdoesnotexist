"""PREPARED G-only native noise-offset ablation; no training/image postprocessing.

Loads checkpoint100 raw G, never EMA/D models, preserving source_flattened layout.
All four parent latents and all three arms are retained. A controlled inference
...
GIB = 2**30
SINGLE_OFFSET = 'synthesis.b1024.conv0.parametrizations.noise_strength.0.b_vector'
...
def prepare(out):
    ...
    sources = [HERE/'noise_ablation.py', HERE/'modulation.py', GMETA,
    ...
    protocol = {'name':'checkpoint100-rawG-noise-offset-ablation', 'checkpoint':str((PARENT/'checkpoint-100.pt').relative_to(ROOT)),
        ...
        'single_offset':SINGLE_OFFSET, 'all_noise_offset_count':17,
    ...
    shutil.copy2(HERE/'noise_ablation.py',out/'noise_ablation.py')
...
def supervise(out):
    ...
    try:
        with (out/'worker.log').open('w') as log:
            proc=subprocess.Popen([sys.executable,str(HERE/'noise_ablation.py'),'--worker','--output',str(out)],
        ...
        if failure is None:
            ...
            assert result['complete'] and len(result['images'])==12
...
def apply_offset_arm(model, original_offsets, reset_names):
    """Restore all noise offsets, then zero explicitly named offsets only."""
    import torch
...
def worker(out):
    ...
    offsets={n:p.detach().clone() for n,p in g.named_parameters() if n.endswith('.parametrizations.noise_strength.0.b_vector')}
    assert len(offsets)==17 and SINGLE_OFFSET in offsets
    assert all(t.numel()==1 for t in offsets.values())
    for name in offsets:
        original_name=name.replace('.0.b_vector','.original')
        assert original_name in original_hashes
    noise_values={n:{'offset':float(t),'original':float(g.get_parameter(n.replace('.0.b_vector','.original')))} for n,t in offsets.items()}
    write(out/'loaded-state.json',{'load_seconds':load_seconds,'state_sha256_by_tensor':original_hashes,
        'noise_offsets':noise_values,'checkpoint_sha256':CHECKPOINT_SHA,'layout':inventory['conv_layout']})
    ...
    with torch.no_grad():
        for arm in ARMS:
            reset=[] if arm==ARMS[0] else [SINGLE_OFFSET] if arm==ARMS[1] else list(offsets)
            apply_offset_arm(g,offsets,reset)
            hashes=state_hashes()
            changed=[n for n in hashes if hashes[n]!=original_hashes[n]]
            assert set(changed)<=set(reset), 'Intervention changed undeclared model state'
            assert all(not torch.count_nonzero(g.get_parameter(n)) for n in reset)
            if arm==ARMS[0]:assert hashes==original_hashes
            ...
            for i,latent in enumerate(z):
                ...
                if arm==ARMS[0]:
                    expected=protocol['archived_raw_png_sha256'][f'raw-{i:03}.png']
                    with Image.open(PARENT/'step-100'/f'raw-{i:03}.png') as old:
                        assert np.array_equal(array,np.array(old)), 'Unchanged raw pixels do not reproduce parent'
                    assert record['sha256']==expected, 'Unchanged PNG bytes do not reproduce parent'
                    record['parent_png_exact']=True
    ...
    write(out/'result.json',{'complete':True,'images':images,'arms':arm_records,'original_state_restored_exact':True,
        'unchanged_raw_parent_pngs_exact':True,'protocol_sha256':sha(out/'protocol.json'),
