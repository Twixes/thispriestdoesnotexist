...
from experiments.cdc.cdc_loss import cdc_loss

CDC_BLOCKS = (32, 64, 128)
CDC_SEED = 20260921
...
def main():
    ...
    p.add_argument('--cdc-weight', type=float, default=0, help='Optional source correspondence regularizer; zero disables it')
    p.add_argument('--cdc-batch', type=int, default=4, help='Independent unmixed CDC latent batch (at least four)')

    args=p.parse_args()
    if args.freeze_d_layers < 0:
        p.error('--freeze-d-layers must be non-negative')
    if not math.isfinite(args.cdc_weight) or args.cdc_weight < 0:
        p.error('--cdc-weight must be finite and non-negative')
    if args.cdc_batch < 4:
        p.error('--cdc-batch must be at least four')
    ...
    # Capture the ORIGINAL source before a resumed adapted G can overwrite it.
    cdc_source=copy.deepcopy(nets['G']).to(args.device).eval().requires_grad_(False) if args.cdc_weight else None
    cdc_rng=torch.Generator(device='cpu').manual_seed(CDC_SEED)
    ...
    if args.resume:
        ...
        if checkpoint.get('cdc_weight',0) and args.cdc_weight:
            if checkpoint.get('cdc_batch') != args.cdc_batch or checkpoint.get('cdc_blocks') != list(CDC_BLOCKS):
                raise ValueError('Resume CDC feature/batch mismatch; preserve the correspondence configuration')
            if checkpoint.get('cdc_base_sha256') != hashlib.sha256(args.base.read_bytes()).hexdigest():
                raise ValueError('Resume CDC source checkpoint mismatch')
            if 'cdc_rng' not in checkpoint:
                raise ValueError('Enabled CDC checkpoint lacks its independent latent RNG state')
        if 'cdc_rng' in checkpoint:
            cdc_rng.set_state(checkpoint['cdc_rng'])
    ...
    if args.smoke_test:
        g_trainable_name,g_trainable_tensor=next(iter(g.named_parameters()))
        g_trainable_before=g_trainable_tensor.detach().cpu().clone()
        cdc_source_before=state_hash(cdc_source) if cdc_source is not None else None
        cdc_memory={}
        if args.device=='mps':
            torch.mps.synchronize()
            cdc_memory['before_training']={'current':torch.mps.current_allocated_memory(),'driver':torch.mps.driver_allocated_memory()}
    ...
    config['cdc']={'weight':args.cdc_weight,'batch':args.cdc_batch,'blocks':list(CDC_BLOCKS),
                   'pool_size':8,'temperature':1.0,'seed':CDC_SEED,'source':'base raw G before resume',
                   'source_base_sha256':config['base_sha256'],'noise':'const','latents':'unmixed, dedicated CPU generator',
                   'helper_sha256':hashlib.sha256((ROOT/'experiments/cdc/cdc_loss.py').read_bytes()).hexdigest()}
    ...
    for step in range(start_step+1,args.steps+1):
        ...
        loss_g.backward()
        # Release the adversarial graph before constructing the separate CDC graph.
        loss_g=loss_g.detach()
        del fake
        loss_cdc=torch.zeros((),device=args.device)
        cdc_details=None
        if cdc_source is not None:
            if args.smoke_test:
                cpu_rng_before_cdc=torch.get_rng_state()
                mps_rng_before_cdc=torch.mps.get_rng_state() if args.device=='mps' else None
            cdc_z=torch.randn(args.cdc_batch,g.z_dim,generator=cdc_rng).to(args.device)
            loss_cdc,cdc_details=cdc_loss(cdc_source,g,cdc_z,blocks=CDC_BLOCKS)
            if not torch.isfinite(loss_cdc):raise RuntimeError(f'Non-finite CDC loss at step {step}')
            (loss_cdc*args.cdc_weight).backward()
            loss_cdc=loss_cdc.detach()
            if args.smoke_test:
                assert torch.equal(cpu_rng_before_cdc,torch.get_rng_state()), 'CDC changed global CPU RNG'
                if args.device=='mps':
                    assert torch.equal(mps_rng_before_cdc,torch.mps.get_rng_state()), 'CDC changed adversarial MPS RNG'
            if args.smoke_test and args.device=='mps':
                torch.mps.synchronize()
                cdc_memory['after_cdc_backward']={'current':torch.mps.current_allocated_memory(),'driver':torch.mps.driver_allocated_memory()}
        if args.smoke_test:
            assert all(torch.isfinite(value.grad).all() for value in g.parameters() if value.grad is not None), 'Non-finite G gradient'
        g_opt.step()
        ...
        if step%10==0 or args.smoke_test:
            record={'step':step,'kimg':(prior_images_seen+(step-start_step)*args.batch)/1000,'seconds':time.monotonic()-started,
                    'g_loss':float(loss_g.detach()),'cdc_loss':float(loss_cdc),'cdc_weighted_loss':float(loss_cdc)*args.cdc_weight,
                    'cdc_details':cdc_details,'d_loss':float(loss_d.detach()),'r1':float(r1.detach()),'augment_p':float(aug_p),
        ...
        if not args.smoke_test and (step%args.snapshot_every==0 or step==args.steps):
            ...
            checkpoint={'G':cpu_state(g),'D':cpu_state(d),'G_ema':cpu_state(ema),
                        ...
                        'cdc_weight':args.cdc_weight,'cdc_batch':args.cdc_batch,'cdc_blocks':list(CDC_BLOCKS),
                        'cdc_base_sha256':config['base_sha256'],'cdc_rng':cdc_rng.get_state(),'cdc_config':config['cdc'],
    ...
    if args.smoke_test:
        ...
        assert not torch.equal(g_trainable_before,dict(g.named_parameters())[g_trainable_name].detach().cpu()), 'Trainable G weight did not update'
        if cdc_source is not None:
            assert state_hash(cdc_source)==cdc_source_before, 'Frozen CDC source changed'
            assert all(value.grad is None and not value.requires_grad for value in cdc_source.parameters()), 'CDC source received gradients'
        if args.device=='mps':
            torch.mps.synchronize()
            cdc_memory['after_update']={'current':torch.mps.current_allocated_memory(),'driver':torch.mps.driver_allocated_memory()}
        evidence={'step':step,'resumed':bool(args.resume),'freeze_d_layers':args.freeze_d_layers,
                  ...
                  'g_trainable_tensor_updated':g_trainable_name,'g_gradients_finite':True,
                  'cdc_weight':args.cdc_weight,'cdc_loss':float(loss_cdc),
                  'cdc_source_unchanged':cdc_source is not None and state_hash(cdc_source)==cdc_source_before,
                  'cdc_source_has_no_gradients':cdc_source is not None and all(value.grad is None for value in cdc_source.parameters()),
                  'cdc_source_sha256':cdc_source_before,'cdc_preserves_adversarial_rng':cdc_source is not None,'mps_memory_bytes':cdc_memory,
