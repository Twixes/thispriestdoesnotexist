...
def main():
    output = RUN/'checkpoint-003500-validation.json'
    integrity_path = RUN/'checkpoint-003500-validation-integrity.json'
    ...
    (RUN/'checkpoint-003500-memory-before.txt').write_text(pressure)
    ...
    files = [RUN/'resume.pt', RUN/'generator-003500.pt', validator]
    before = {str(p.relative_to(ROOT)):sha(p) for p in files}
    import torch
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    sys.argv = [str(validator), '--resume', str(RUN/'resume.pt'), '--generator',
                str(RUN/'generator-003500.pt'), '--step', '3500', '--output', str(output)]
    with (RUN/'checkpoint-003500-validation.log').open('w') as log, contextlib.redirect_stdout(log):
    ...
    result = {
        ...
        'step':3500,'memory_preflight_free_percent':free,'device':'cpu',
