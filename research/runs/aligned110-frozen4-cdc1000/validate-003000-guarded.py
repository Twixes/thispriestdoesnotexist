...
def main():
    output = RUN/'checkpoint-003000-validation.json'
    integrity_path = RUN/'checkpoint-003000-validation-integrity.json'
    ...
    (RUN/'checkpoint-003000-memory-before.txt').write_text(pressure)
    ...
    files = [RUN/'resume.pt', RUN/'generator-003000.pt', validator]
    before = {str(p.relative_to(ROOT)):sha(p) for p in files}
    import torch
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    sys.argv = [str(validator), '--resume', str(RUN/'resume.pt'), '--generator',
                str(RUN/'generator-003000.pt'), '--step', '3000', '--output', str(output)]
    with (RUN/'checkpoint-003000-validation.log').open('w') as log, contextlib.redirect_stdout(log):
    ...
    result = {
        ...
        'step':3000,'memory_preflight_free_percent':free,'device':'cpu',
        ...
        'note':'Stable at validation time only; resume.pt is live and can be atomically replaced later.',
    }
