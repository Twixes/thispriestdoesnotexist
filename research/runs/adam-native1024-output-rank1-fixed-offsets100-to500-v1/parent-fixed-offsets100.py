"""Guarded 100-iteration native1024 output-rank1 FIXED-NOISE-AND-SYNTHESIS-BIAS stability experiment.
...
def prepare(out):
    ...
    assert sha(HERE/'probe_output_rank1_fixed_noise100.py') == FIXED_NOISE_PARENT_SHA256
    sources = [HERE/'probe_output_rank1_fixed_offsets100.py', HERE/'probe_output_rank1_fixed_noise100.py', EVAL_Z_SOURCE, HERE/'probe_output_rank1.py', HERE/'probe.py', HERE/'smoke.py', HERE/'modulation.py', HERE/'upstream/provenance.json', GDIR/'model.json', DDIR/'metadata.json']
    ...
    protocol = {'name':'adam-native1024-output-rank1-fixed-offsets100-stability', 'conv_layout':'output_rank1', 'derived_from_probe_sha256':SOURCE_PROBE_SHA256, 'iterations':ITERATIONS, 'checkpoint_steps':list(CHECKPOINT_STEPS), 'derived_from_smoke_sha256':SMOKE_SHA256, 'resolution':1024,
        ...
        'deviations':['Output-rank1 convolution layout differs from released AdAM source_flattened control',
                      'All17 original G noise strengths and17 synthesis activation biases retained;34 additive offsets fixed zero/excluded from optimizer, unlike AdAM',
        ...
        'fixed_offset_policy':{'offset_count':34,'noise_count':17,'synthesis_bias_count':17,'offset_value':0,'optimizer_excluded':True,'raw_and_ema_fixed':True,'original_strengths_and_biases_unchanged':True},
        'derived_from_output_rank1_probe_sha256':OUTPUT_RANK1_PROBE_SHA256,
        'derived_from_fixed_noise_parent_sha256':FIXED_NOISE_PARENT_SHA256,
        'production_approved':False,'server_latency_proven':False}
    out.mkdir(parents=True)
    (out/'.gitignore').write_text('*.partial\n')
    write(out/'protocol.json', protocol)
    shutil.copy2(HERE/'probe_output_rank1_fixed_offsets100.py',out/'probe_output_rank1_fixed_offsets100.py')
    shutil.copy2(HERE/'probe_output_rank1_fixed_noise100.py',out/'source-fixed-noise100.py')
...
def supervise(out):
    ...
    try:
        with (out/'worker.log').open('w') as log:
            proc = subprocess.Popen([sys.executable, str(HERE/'probe_output_rank1_fixed_offsets100.py'),'--worker','--output',str(out)],
...
def worker(out):
    ...
    def previews(step):
        ...
        write(folder/'manifest.json',{'complete':True,'step':step,'protocol_sha256':sha(out/'protocol.json'),
            'eval_z_sha256':sha(out/'eval-z.npz'),'rng_unchanged':True,'raw_and_ema_offsets_fixed':True,
