"""Root-reviewed four-arm local experiment launch; records terminal result, never retries."""
...
command=['/usr/bin/time','-l','caffeinate','-i',str(root/'research/.venv/bin/python'),str(exp/'runner.py'),'--execute','--output',str(run)]
record={'command':command,'cwd':str(root),'started_unix':time.time(),'source_files':hashes,'purpose':'Four sequential300-update arms from original600; root-full-run-approval.json records reviewed smoke and fixed resource/objective choices.','production_approved':False}
