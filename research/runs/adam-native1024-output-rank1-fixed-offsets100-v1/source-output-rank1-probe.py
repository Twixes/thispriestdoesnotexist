"""Guarded output-rank-one 500-iteration native1024 importance-probing experiment.
...
def prepare(out):
    ...
    sources = [HERE/'probe_output_rank1.py', HERE/'probe.py', HERE/'smoke.py', HERE/'modulation.py', HERE/'upstream/provenance.json', GDIR/'model.json', DDIR/'metadata.json']
    ...
    protocol = {'name':'adam-native1024-output-rank1-probing500', 'conv_layout':'output_rank1', 'derived_from_probe_sha256':SOURCE_PROBE_SHA256, 'iterations':ITERATIONS, 'checkpoint_steps':list(CHECKPOINT_STEPS), 'derived_from_smoke_sha256':SMOKE_SHA256, 'resolution':1024,
        ...
        'preview_seed':2026092210, 'preview_count':4, 'preview_steps':[0,*CHECKPOINT_STEPS],
        ...
        'deviations':['NVIDIA native1024 networks rather than Rosinality256; preserve own equalized scaling',
                      ...
                      'Exactly500 probing iterations; no Fisher/main adaptation and no convergence claim'],
        'guards':{'start_available_fraction':.35,'runtime_available_fraction':.20,'rss_gib':12,'swap_growth_mib':512,'seconds':7200},
        'initialization':'fresh original G_ema and D, not the smoke checkpoint',
        'execution_policy':'Two-hour wall deadline from supervisor start; no restart, resume, changed objective or reduced-resolution fallback',
    ...
    shutil.copy2(HERE/'probe_output_rank1.py',out/'probe_output_rank1.py')
...
def supervise(out):
    ...
    try:
        with (out/'worker.log').open('w') as log:
            proc = subprocess.Popen([sys.executable, str(HERE/'probe_output_rank1.py'),'--worker','--output',str(out)],
...
def worker(out):
    ...
    def previews(step):
        ...
        write(folder/'manifest.json',{'complete':True,'step':step,'protocol_sha256':sha(out/'protocol.json'),
            'eval_z_sha256':sha(out/'eval-z.npz'),'rng_unchanged':True,
    ...
    for iteration in range(ITERATIONS):
        ...
        record.update(seconds=time.monotonic()-tick,path_mean=float(path_mean),optimizer_steps=dict(optimizer_steps))
