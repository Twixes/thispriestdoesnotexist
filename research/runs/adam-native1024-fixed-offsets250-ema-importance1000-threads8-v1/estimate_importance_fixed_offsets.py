...
def prepare(run, step, pairs, out):
    ...
    for path in (HERE / 'estimate_importance_fixed_offsets.py', HERE / 'importance.py', HERE / 'fixed_offset_policy.py',
    ...
    for name in ('estimate_importance_fixed_offsets.py', 'importance.py', 'modulation.py', 'fixed_offset_policy.py'):
...
def supervise(run, step, pairs, out):
    ...
    try:
        with (out / 'worker.log').open('w') as log:
            proc = subprocess.Popen([sys.executable, str(HERE / 'estimate_importance_fixed_offsets.py'),
