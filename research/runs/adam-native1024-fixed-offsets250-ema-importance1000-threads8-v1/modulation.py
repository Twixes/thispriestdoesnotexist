...
def fold_modulation(model: nn.Module, *, inplace: bool = False) -> nn.Module:
    ...
    for module in list(folded.modules()):
        if parametrize.is_parametrized(module):
            # deepcopy preserves the generated parametrized CLASS. PyTorch's
            # remove_parametrizations deletes properties from that class, which
            # would break the original model (or another deepcopy). Give this
            # instance its own property-owning class before any removals. Keep
            # the original bases so PyTorch restores the real NVIDIA class.
            current_class = module.__class__
            isolated_attributes = {name: value for name, value in current_class.__dict__.items()
                                   if name not in ("__dict__", "__weakref__")}
            module.__class__ = type(current_class.__name__, current_class.__bases__, isolated_attributes)
