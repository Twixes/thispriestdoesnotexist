"""Exercise recovery cleanup using temporary fixtures and harmless sleep groups only."""
import hashlib
...
SOURCE = ROOT / 'research/runs/recover-factorial-CD.py'
...
def scenario(name, directory):
    ...
    run = directory / 'fake-run'
    run.mkdir()
    expected = {'fixture': 'immutable-no-models'}
    dump(Path(str(run) + '.launch.json'), {'exit_code': 1, 'source_files': expected})
    Path(str(run) + '.log').write_text('Defer: at least 35% free memory required\n')
    dump(run / 'source-hashes.json', expected)
    dump(run / 'launch.json', {'deadline_unix': time.time() + (1.0 if name == 'deadline' else 10)})
    for arm in 'AB':
        (run / arm).mkdir()
        dump(run / arm / 'complete.json', {'complete': True, 'arm': arm, 'smoke': False,
            'updates_per_trajectory': 300, 'actual_optimizer_calls': 300,
            'full_checkpoint_roundtrip': True, 'source_and_frozen_checks_passed': True,
            'final_source30_count': 30, 'source_files': expected, 'schedule': []})
    module.ROOT = directory
    module.RUN = run
    module.EXPERIMENT = directory / 'nonexistent-model-code'
    module.own_hashes = lambda: expected.copy()
    module.memory_guard = lambda: 'Fixture memory check: no actual model allocation\n'
    real_popen = subprocess.Popen

    def dummy_popen(command, **kwargs):
        assert command[0] == '/usr/bin/time' and kwargs['start_new_session'] is True
    ...
    module.subprocess = SimpleNamespace(Popen=dummy_popen, TimeoutExpired=subprocess.TimeoutExpired, STDOUT=subprocess.STDOUT)
    if name == 'write_failure':
        ...
        def fail_after_spawn(path, value):
            nonlocal failed
            if value.get('children') and 'wrapper_pid' in value['children'][0] and not failed:
                failed = True
                raise OSError('Injected record-write failure after dummy spawn')
            return original_write(path, value)

        module.write = fail_after_spawn
    signal.signal(signal.SIGINT, signal.default_int_handler)
    sys.argv = [str(SOURCE), '--execute']
    module.main()
...
def main():
    ...
    results = []
    for name in ['write_failure', 'sigint', 'sigterm', 'deadline']:
        with tempfile.TemporaryDirectory(prefix='priest-recovery-lifecycle-') as temporary:
            ...
            try:
                ...
                record = json.loads((directory / 'fake-run/recovery-CD.launch.json').read_text())
                assert all(absent.values()) and no_group, (name, ids, absent, no_group)
                assert exit_code != 0 and record['complete'] is False
                assert len(record['children']) == 1 and record['children'][0]['arm'] == 'C'
                assert record['children'][0]['exit_code'] == -signal.SIGKILL
                assert not (directory / 'fake-run/D.log').exists()
                assert not (directory / 'fake-run/comparison.json').exists()
                if name == 'sigterm':
                ...
                if name == 'deadline':
                    assert 'Original total deadline expired' in record['error']
                results.append({'scenario': name, 'passed': True, 'wall_seconds': time.monotonic()-tick,
                    ...
                    'dummy_group_absent': no_group, 'child_reaped_returncode': record['children'][0]['exit_code'],
                    'recorded_error': record['error'], 'only_C_spawned': True, 'no_comparison_published': True,
    ...
    evidence = {'passed': True, 'tested_source': str(SOURCE.relative_to(ROOT)), 'source_sha256': digest,
        ...
        'fixture_policy': 'Temporary fake run/provenance, fake memory guard; Popen substitutes only harmless Python sleep group. Recovery main/cleanup/wait/signal handlers are real unmodified code.',
        'results': results}
    output = ROOT / 'research/reviews/factorial-recovery-lifecycle-evidence.json'
    if output.exists():
