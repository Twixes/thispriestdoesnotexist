...
The command above is documentation only; evaluation has not run. Expect 30 results and zero additional exclusions. For native male review use 004, 025, 028, 017 and 024; the last two add beard/glasses and cropped-neck difficulties. 000 and 030 belong in the training review, never the unseen-success count. The generic fixed top75/bottom 25 MAE remains a preservation diagnostic, not identity, attractiveness or collar recognition. It does not provide target loss for all six training pairs; that end-of-run paired review must be added separately without changing the trainer.
