...
The isolated native1024 [NADA-style CPU smoke](runs/nada-clean24-cpu-smoke10/) completed ten updates in77.5 seconds, with roughly2.13 seconds per update and6.61GiB peak worker RSS. It reproduced all32 source images exactly, preserved frozen state, passed finite-gradient checks, and restored full optimizer/RNG state with an identical generated preview. Ten steps establish compute feasibility only; the images still largely resemble the original FFHQ faces and are not a priest generator. An earlier [overlap attempt](runs/nada-clean24-overlap-cpu-smoke10/failure-review.json) stopped because root launched a metadata-only validation check that tripped its conservative competing-process guard; it provides no model-training result.

A newer [36-image whole-portrait dataset](data/plain-background36-whole-image/README.md) is available for future experiments. It retains24 previously selected originals and adds12 from16 newly generated/reviewed portraits. The NADA smoke uses the original24, unchanged.
